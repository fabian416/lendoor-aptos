...
import { useDemoteJunior } from '@/hooks/junior/useWithdrawJunior'
...
export function WithdrawSUSDCPanel({
  ...
  const { submit: submitWithdrawJunior, submitting } = useDemoteJunior();
  const { juniorWithdrawAvailableDisplay: juniorAvail } = useUser();

  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!isLoggedIn) return onConnect()
    const ok = await submitWithdrawJunior(amount)
    if (ok) {
      onWithdraw?.(amount)
      setAmount('')
    }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-2 w-full mx-auto">
        ...
        <AvailableToWithdrawKPI value={`${juniorAvail}`} />
        <JusdcBalanceKPI />
      </div>
