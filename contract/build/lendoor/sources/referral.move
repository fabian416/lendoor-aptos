...
module aries::referral {
    ...
    public fun new_referral_details(): ReferralDetails {
        ReferralDetails { configs: table::new() }
    }
    ...
    public(friend) fun register_or_update_privileged_referrer(
        ...
        if (table::contains(&referral_details.configs, claimant)) {
            let cfg = table::borrow_mut(&mut referral_details.configs, claimant);
            cfg.fee_sharing_percentage = fee_sharing_percentage;
        } else {
            table::add(
                &mut referral_details.configs,
                claimant,
                ReferralConfig { fee_sharing_percentage }
            );
        };
    }
